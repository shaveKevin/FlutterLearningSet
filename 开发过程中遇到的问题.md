...
```
根据hero的定义我们可以知道，在一个页面中，只能存在一个相同的tag值，多个heroes不能使用相同的tag值。而我们的`floating action button `就是一个`hero`，并且一个页面出现了两次，如果不设置`heroTag`就会产生冲突。我们对这个两个`FloatingActionButton`添加`heroTag`之后，冲突就解决了。(这个herotag主要是方便做动画效果，具体可参考hero动画)
